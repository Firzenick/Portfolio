		...
		Camera* camera)
{
	...
	// NEW: Check which camera processForeground is being called for, apply threshold() with manually decided Hue threshold
	if (camera == m_cameras[0]) {
		threshold(tmp, foreground, 10, 255, CV_THRESH_BINARY);
	} else if (camera == m_cameras[1]) {
		threshold(tmp, foreground, 5, 255, CV_THRESH_BINARY);
	} else if (camera == m_cameras[2]) {
		threshold(tmp, foreground, 8, 255, CV_THRESH_BINARY);
	} else if (camera == m_cameras[3]) {
		threshold(tmp, foreground, 5, 255, CV_THRESH_BINARY);
	}
	else {
	...
	// NEW: Check which camera processForeground is being called for, apply threshold() with manually decided Saturation threshold
	if (camera == m_cameras[0]) {
		threshold(tmp, background, 15, 255, CV_THRESH_BINARY);
	}
	else if (camera == m_cameras[1]) {
		...
		threshold(tmp, background, 35, 255, CV_THRESH_BINARY);
	}
	else if (camera == m_cameras[3]) {

		threshold(tmp, background, 20, 255, CV_THRESH_BINARY);
	}
	else {
	...
	// NEW: Check which camera processForeground is being called for, apply threshold() with manually decided Value threshold
	if (camera == m_cameras[0]) {
		threshold(tmp, background, 40, 255, CV_THRESH_BINARY);
	}
	else if (camera == m_cameras[1]) {
		threshold(tmp, background, 45, 255, CV_THRESH_BINARY);
	}
	else if (camera == m_cameras[2]) {
		threshold(tmp, background, 35, 255, CV_THRESH_BINARY);
	}
...
void Scene3DRenderer::createFloorGrid()
{
	...
	for (int y = -size * m_num; y <= size * m_num; y += size)
		edge1.push_back(new Point3i(-size * m_num / 4, y, z_offset));

	// edge 2
	vector<Point3i*> edge2;
	for (int x = -size * m_num / 4; x <=  7 * size * m_num / 4; x += size)
		edge2.push_back(new Point3i(x, size * m_num, z_offset));

	// edge 3
	vector<Point3i*> edge3;
	for (int y = -size * m_num; y <= size * m_num; y += size)
		edge3.push_back(new Point3i(7 * size * m_num / 4, y, z_offset));
