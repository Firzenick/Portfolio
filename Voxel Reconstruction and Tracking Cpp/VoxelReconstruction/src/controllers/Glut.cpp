...
namespace nl_uu_science_gmt
{
		...
		int v)
	{
		...
		if (scene3d.getReconstructor().getInitialHistograms().size() == 0)
		{
			for (int i = 0; i < scene3d.getCameras().size(); i++)
			{
				scene3d.getCameras()[i]->setVideoFrame(660);
			}

			scene3d.processFrame();
			scene3d.getReconstructor().init2();

			for (int i = 0; i < scene3d.getCameras().size(); i++)
			{
				scene3d.getCameras()[i]->setVideoFrame(0);
			}
			...
			for (int i = 0; i < scene3d.getCameras().size(); i++)
			{
				scene3d.getReconstructor().m_camera_foregrounds.clear();
			}
		}
	...
	void Glut::drawVoxels()
	{
		...
		vector<vector<GLfloat>> colsByLabel;
		colsByLabel.resize(4);
		colsByLabel[0] = { 1.0f, 0.1f, 0.1f, 0.5f };
		colsByLabel[1] = { 0.1f, 1.0f, 0.1f, 0.5f };
		colsByLabel[2] = { 0.1f, 0.1f, 1.0f, 0.5f };
		colsByLabel[3] = { 1.0f, 0.6f, 0.2f, 0.5f };

		vector<PixelVoxel::Voxel*> voxels = m_Glut->getScene3d().getReconstructor().getVisibleVoxels();
		for (size_t v = 0; v < voxels.size(); v++)
		{
			PixelVoxel::Voxel* voxel = voxels[v];
			glColor4f(colsByLabel[voxel->label][0], colsByLabel[voxel->label][1], colsByLabel[voxel->label][2], colsByLabel[voxel->label][3]);
			glVertex3f((GLfloat)voxels[v]->x, (GLfloat)voxels[v]->y, (GLfloat)voxels[v]->z);
		}

		Mat m = m_Glut->getScene3d().getReconstructor().getClusterCenters();
		for (int i = 0; i < 4; i++)
		{
			vector<Point> v = m_Glut->getScene3d().getReconstructor().getCenterTrails()[i];

			glColor4f(colsByLabel[i][0], colsByLabel[i][1], colsByLabel[i][2], colsByLabel[i][3]);

			for (int j = 0; j < v.size(); j++)
			{
				glVertex3f((GLfloat)v[j].x, (GLfloat)v[j].y, (GLfloat)0);
			}

/*
			float x = m.at<float>(i, 0), y = m.at<float>(i, 1);
			glVertex3f((GLfloat)m.at<float>(i, 0), (GLfloat)m.at<float>(i, 1), (GLfloat)0);
			glVertex3f((GLfloat)m.at<float>(i, 0) + 32, (GLfloat)m.at<float>(i, 1), (GLfloat)0);
			glVertex3f((GLfloat)m.at<float>(i, 0) - 32, (GLfloat)m.at<float>(i, 1), (GLfloat)0);
			glVertex3f((GLfloat)m.at<float>(i, 0), (GLfloat)m.at<float>(i, 1) + 32, (GLfloat)0);
			glVertex3f((GLfloat)m.at<float>(i, 0), (GLfloat)m.at<float>(i, 1) - 32, (GLfloat)0);*/
		}
