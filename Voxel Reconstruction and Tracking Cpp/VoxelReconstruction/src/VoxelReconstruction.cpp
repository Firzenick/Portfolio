		...
		m_data_path(dp), m_cam_views_amount(cva)
{
	...
	for (int v = 0; v < m_cam_views_amount; ++v)
	{
		...
		// NEW: Option to create background images from 128 frames of background.avi
		cout << "Do you want to create new background.png files?";
		string answer;
		cin >> answer;
		if (answer == "y")
		{
			// NEW: Load a different video file
			std::cout << full_path.str() << General::BackgroundVideoFile << std::endl;
			assert(General::fexists(full_path.str() + General::BackgroundVideoFile));

			m_cam_views.push_back(new Camera(full_path.str(), General::ConfigFile, v, true));
		}
		else
		{
			/*
			...
			m_cam_views.push_back(new Camera(full_path.str(), General::ConfigFile, v, false));
		}
	}
}
...
void VoxelReconstruction::run(int argc, char** argv)
{
	for (int v = 0; v < m_cam_views_amount; ++v)
	{
		if (m_cam_views[v]->getBackground())
		{
			// NEW: sum 64 frames (quadrupled) after converting them to a datatype 256 times as large, then write the background.png to the respective cam folder
			bool has_cam = true;
			has_cam = m_cam_views[v]->initialize();
			assert(has_cam);

			int height = m_cam_views[v]->getSize().height, width = m_cam_views[v]->getSize().width;
			Mat total(height, width, CV_16UC3);
			total = Scalar(0, 0, 0);
			Mat frame;
			int number = 0;
			for (int i = 0; i < 64; i++)
			{
				frame = m_cam_views[v]->advanceVideoFrame();
				frame.convertTo(frame, CV_16UC3, 4);
				total += frame;
				number++;
			}

			imwrite("data/cam" + to_string(v + 1) + "/" + General::BackgroundImageFile, total);
		}
		else
		{
